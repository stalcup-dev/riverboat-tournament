import type { FishData, ScoringData } from "./data.js";

export interface ResolveCatchInput {
  matchSeed: number;
  playerId: string;
  castSeq: number;
  hotspotId: string;
  fishCatalog: FishData[];
  scoring: ScoringData;
}

export interface ResolveCatchOutput {
  fish_id: string;
  weight: number;
  length: number;
  rarity_tier: number;
...
export function resolveCatchOutcome(input: ResolveCatchInput): ResolveCatchOutput {
...
export function deriveCastSeed(matchSeed: number, playerId: string, castSeq: number, hotspotId: string): number {
...
function pickFishWeightedByRarity(fishCatalog: FishData[], rng: () => number): FishData {
  ...
  for (let index = 0; index < fishCatalog.length; index += 1) {
    ...
    if (roll <= cursor) {
      return fishCatalog[index] as FishData;
    }
  }

  return fishCatalog[fishCatalog.length - 1] as FishData;
}

function getRarityMult(scoring: ScoringData, rarityTier: number): number {
