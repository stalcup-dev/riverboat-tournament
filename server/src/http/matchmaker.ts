...
import { normalizeCode, type MatchCodeRegistry } from "../matchmaker/registry.js";
...
const ipJoinBuckets = new Map<string, TokenBucket>();
const ipCreateBuckets = new Map<string, TokenBucket>();

export async function tryHandleMatchmaker(
  req: IncomingMessage,
  res: ServerResponse,
  config: RuntimeConfig,
  registry: MatchCodeRegistry
): Promise<boolean> {
...
async function handleJoin(
  ...
  const room = rooms[0] as { clients?: unknown; maxClients?: unknown } | undefined;
...
async function handleLobbyList(req: IncomingMessage, res: ServerResponse, config: RuntimeConfig): Promise<void> {
  if (req.method === "OPTIONS") {
    applyCorsHeaders(res, config.corsOrigin);
    res.statusCode = 204;
    res.end();
  ...
  const allMatches = (await matchMaker.query({ name: "match" })) as Array<{
    roomId?: unknown;
    clients?: unknown;
    maxClients?: unknown;
    metadata?: unknown;
...
function sendJson(res: ServerResponse, status: number, payload: unknown, corsOrigin?: string): void {
...
function applyCorsHeaders(res: ServerResponse, corsOrigin?: string): void {
  if (!corsOrigin) {
    return;
  }
...
function validateOrigin(req: IncomingMessage, res: ServerResponse, config: RuntimeConfig): boolean {
...
function validateInviteKey(req: IncomingMessage, res: ServerResponse, config: RuntimeConfig): boolean {
...
function shouldEnforceMatchmakerSecret(nodeEnv: string, configuredSecret: string): boolean {
