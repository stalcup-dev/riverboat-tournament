...
import { loadFishingDataPack, type FishData, type HotspotData, type ScoringData } from "../fishing/data.js";
import { getHotspotRetryAfterMs, HOTSPOT_COOLDOWN_MS_DEFAULT, markHotspotDepleted } from "../fishing/depletion.js";
import { validateCastStart, validateCatchClick, type FishingErrorCode } from "../fishing/guards.js";
...
import { computeMatchResults, type MatchResultsPayload, type ResultPlayerSnapshot } from "../results/awards.js";
import { validateRoomJoinCode } from "../security/joinCode.js";
import { getOrCreateBucket, TokenBucket } from "../security/rateLimiter.js";
...
interface CatchClickPayload {
  v?: unknown;
  client_seq?: unknown;
  offer_id?: unknown;
}

interface GatherWoodPayload {
  v?: unknown;
  client_seq?: unknown;
}
...
export class MatchRoom extends Room<RoomState> {
  ...
  private readonly roomCreatedAtMs = Date.now();
  private readonly lastPingAtBySessionId = new Map<string, number>();
  private readonly rttEstimateBySessionId = new Map<string, number>();
  private readonly activeFishersByHotspot = new Map<string, number>();
  private fishingHotspotsById = new Map<string, HotspotData>();
  private fishCatalog: FishData[] = [];
  private scoringConfig: ScoringData = { weight_k: 0.1, length_k: 0.01, rarity_mults: { "1": 1 } };
  private matchSeed = 1;
  private spawnPoint = { x: SPAWN_FALLBACK_X, y: SPAWN_FALLBACK_Y };
  private readonly lastWaterGateErrorAtBySessionId = new Map<string, number>();
  private readonly speciesCaughtByPlayerId = new Map<string, Set<string>>();
  private readonly pointsHistoryByPlayerId = new Map<string, Array<{ atMs: number; points: number }>>();
  private readonly depletedUntilByHotspotId = new Map<string, number>();
  private readonly moveRateLimitBySession = new Map<string, TokenBucket>();
  private readonly castRateLimitBySession = new Map<string, TokenBucket>();
  private readonly catchRateLimitBySession = new Map<string, TokenBucket>();
  private readonly joinRateLimitBySession = new Map<string, TokenBucket>();
  private readonly rateLimitViolationsBySession = new Map<string, number>();
  private lastMatchResults: MatchResultsPayload | null = null;
  private resultsEmittedForCurrentMatch = false;
  ...
  onCreate(_options: unknown): void {
    ...
    console.info(
      `[room:${this.roomId}] created zones=${this.state.zones.length} hotspots=${this.fishingHotspotsById.size} fish=${this.fishCatalog.length} match_seed=${this.matchSeed}`
    );

    this.onMessage("JOIN", (client: Client, message: JoinPayload) => {
      ...
      if (decision.kind === "resume") {
        ...
        console.info(
          `[room:${this.roomId}] resume name=${canonicalName} old_session_id=${decision.fromSessionId} new_session_id=${client.sessionId}`
        );
        return;
      }
    ...
    this.onMessage("MOVE", (client: Client, message: MovePayload) => {
      ...
      if (
        (message.dx !== undefined && (typeof message.dx !== "number" || !Number.isFinite(message.dx))) ||
    ...
    this.onMessage("GATHER_WOOD", (client: Client, _message: GatherWoodPayload) => {
    ...
    this.onMessage("BUILD_CANOE", (client: Client, message: EnvelopePayload) => {
    ...
    this.onMessage("CAST_START", (client: Client, message: CastStartPayload) => {
    ...
    this.onMessage("CATCH_CLICK", (client: Client, message: CatchClickPayload) => {
    ...
    this.onMessage("HOST_START_MATCH", (client: Client, _message: HostControlPayload) => {
    ...
    this.onMessage("HOST_CANCEL_COUNTDOWN", (client: Client, _message: HostControlPayload) => {
    ...
    this.onMessage("PING", (client: Client, message: PingPayload) => {
  ...
  onLeave(client: Client, _consented: boolean): void {
    ...
    console.info(
      `[room:${this.roomId}] leave session_id=${client.sessionId} clients=${this.clients.length} grace_ms=${DISCONNECT_GRACE_MS}`
    );
  }

  private initializeWorldFromData(): void {
  ...
  private processFishingTick(nowMs: number): void {
  ...
  private processPhaseTick(nowMs: number): void {
    if (this.state.phase === "MATCH") {
  ...
  private issueBiteOffer(player: PlayerState, nowMs: number): void {
  ...
  private beginCastCooldown(player: PlayerState, nowMs: number, keepOfferId: boolean): void {
  ...
  private resetCastState(player: PlayerState): void {
    player.cast_state = "IDLE";
    player.cast_hotspot_id = "";
    player.cast_offer_id = "";
    player.cast_expires_ms = 0;
  ...
  private recomputeActiveFishers(): void {
    this.activeFishersByHotspot.clear();
  ...
  private getCatchGraceMs(sessionId: string): number {
    const rttMs = this.rttEstimateBySessionId.get(sessionId);
    return computeCatchGraceMs(rttMs, CATCH_GRACE_MS_DEFAULT);
  }

  private sendError(client: Client, payload: ErrorPayload): void {
    client.send("ERROR", payload);
  }

  private sendFishingError(client: Client, code: FishingErrorCode): void {
    this.sendError(client, {
      code,
  ...
  private getFishingErrorMessage(code: FishingErrorCode): string {
    switch (code) {
  ...
  private rejectJoinAttempt(client: Client, payload: ErrorPayload): void {
  ...
  private cleanupDisconnectedPlayers(nowMs: number): void {
  ...
  private initializePlayerRuntimeState(sessionId: string): void {
    if (!this.speciesCaughtByPlayerId.has(sessionId)) {
      this.speciesCaughtByPlayerId.set(sessionId, new Set<string>());
    }
  ...
  private rebindPlayerRuntimeState(previousSessionId: string, nextSessionId: string): void {
    const species = this.speciesCaughtByPlayerId.get(previousSessionId);
    this.speciesCaughtByPlayerId.delete(previousSessionId);
    this.speciesCaughtByPlayerId.set(nextSessionId, species ?? new Set<string>());
  ...
  private recordSpeciesCatch(playerId: string, fishId: string): boolean {
    const species = this.speciesCaughtByPlayerId.get(playerId) ?? new Set<string>();
  ...
  private getSpeciesCaughtCount(playerId: string): number {
    return this.speciesCaughtByPlayerId.get(playerId)?.size ?? 0;
  }

  private recordPointsAndComputeLast3Min(playerId: string, nowMs: number, pointsDelta: number): number {
  ...
  private validateInboundEnvelope(client: Client, eventName: string, payload: unknown): payload is EnvelopePayload {
    ...
    const envelope = payload as EnvelopePayload;
  ...
  private registerRateViolation(client: Client, reason: string): void {
  ...
  private emitMatchResultsIfNeeded(): void {
    if (this.resultsEmittedForCurrentMatch) {
      return;
    }

    const resultPlayers: ResultPlayerSnapshot[] = [];
    this.state.players.forEach((player) => {
      resultPlayers.push({
        name: player.name,
  ...
  private emitMatchState(targetClient?: Client): void {
  ...
  private scheduleMatchStateReliabilityEmits(): void {
  ...
  private updateRoomStatusMetadata(status: "WAITING" | "IN_PROGRESS"): void {
    this.setMetadata({
      status,
