...
import { applyGatherWood, chooseSpawnPoint, isInsideZoneKind, validateGatherWood, validateWaterEntry } from "../src/world/gameplay.js";
...
test("S5 map zones include FOREST/WATER and rects stay within world bounds", () => {
    ...
    loaded.zones.forEach((zone) => {
        ...
        assert.equal(isRectWithinWorld(zone.rect, loaded.world.width, loaded.world.height), true, `zone ${zone.id} must fit inside world bounds`);
    });
    assert.equal(hasForest, true);
    assert.equal(hasWater, true);
});
test("gather wood validation checks forest membership and cooldown", () => {
    assert.equal(validateGatherWood({
        nowMs: 10_000,
